module Nunchaku::ResourcesInlineEditorHelper

  def inline_editor_for(resource, template)
    @editor ||= inline_editor_class_for(resource.class.name).new(resource, editable_link_resource(resource), editable_link_url(resource), template)
  end
